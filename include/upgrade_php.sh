...
Start_Upgrade_PHP()
{
    ...
    if [ -s php-${php_version}.tar.bz2 ]; then
        ...
        Get_Country
        # Always use official PHP download sources first for security
        Download_Files https://www.php.net/distributions/php-${php_version}.tar.bz2 php-${php_version}.tar.bz2
        if [ $? -ne 0 ]; then
            # Try .edu university mirrors for better speed and reliability
            Download_Files https://mirror.math.princeton.edu/pub/php/releases/php-${php_version}.tar.bz2 php-${php_version}.tar.bz2
        fi
        if [ $? -ne 0 ]; then
            Download_Files https://mirror.cs.uchicago.edu/php/php-${php_version}.tar.bz2 php-${php_version}.tar.bz2
        fi
        if [ $? -ne 0 ]; then
...
Install_PHP_Dependent()
{
    ...
    if echo "${CentOS_Version}" | grep -Eqi "^7" || echo "${RHEL_Version}" | grep -Eqi "^7"  || echo "${Aliyun_Version}" | grep -Eqi "^2" || echo "${Alibaba_Version}" | grep -Eqi "^2" || echo "${Oracle_Version}" | grep -Eqi "^7" || echo "${Anolis_Version}" | grep -Eqi "^7"; then
        if [ "${DISTRO}" = "Oracle" ]; then
            ...
            yum -y install epel-release
            # Use .edu university mirrors for better reliability
            if [ "${country}" = "CN" ]; then
                sed -e 's!^metalink=!#metalink=!g' \
                    -e 's!^#baseurl=!baseurl=!g' \
                    -e 's!//download\.fedoraproject\.org/pub!//mirror.chpc.utah.edu!g' \
                    -e 's!//download\.example/pub!//mirror.chpc.utah.edu!g' \
                    -i /etc/yum.repos.d/epel*.repo
            else
                # Use university mirrors for international users
                sed -e 's!^metalink=!#metalink=!g' \
                    -e 's!^#baseurl=!baseurl=!g' \
                    -e 's!//download\.fedoraproject\.org/pub!//mirror.chpc.utah.edu!g' \
                    -e 's!//download\.example/pub!//mirror.chpc.utah.edu!g' \
        ...
        # Install PHP 8.4 required dependencies with minimum versions
        echo "Installing oniguruma (required for PHP 8.4 mbstring)..."
        yum -y install oniguruma oniguruma-devel
        if [ "${CheckMirror}" = "n" ]; then
            rpm -ivh ${cur_dir}/src/oniguruma-6.8.2-1.el7.x86_64.rpm ${cur_dir}/src/oniguruma-devel-6.8.2-1.el7.x86_64.rpm
        fi
        echo "Installing additional PHP 8.4 dependencies..."
        yum -y install libsodium-devel cmake3
        yum -y install libc-client-devel uw-imap-devel
        # Ensure cmake minimum version for libzip compilation
...
Check_PHP84_Dependencies()
{
    echo "Checking PHP 8.4 specific dependencies..."
    
    # Check libzip version (minimum 0.11 required)
    if ! pkg-config --exists libzip; then
        echo "libzip not found, will be built during compilation"
    else
        libzip_version=$(pkg-config --modversion libzip)
        echo "Found libzip version: $libzip_version"
    fi
    
    # Check libicu version (minimum 50.1 required for PHP 8.4)
    if ! pkg-config --exists icu-i18n; then
        echo "Warning: ICU library not found, installing from university mirrors..."
        if [ "$PM" = "yum" ]; then
        ...
        echo "Found ICU version: $icu_version"
        # Version check for ICU 50.1+
        if ! echo "$icu_version" | grep -Eq '^(5[0-9]\.|[6-9][0-9]\.|[1-9][0-9][0-9]\.)'; then
            echo "Warning: ICU version $icu_version may be too old for PHP 8.4 (requires 50.1+)"
        fi
    fi
    
    # Check oniguruma
    if ! pkg-config --exists oniguruma; then
        echo "Warning: oniguruma not found - mbstring extension may fail"
    else
        onig_version=$(pkg-config --modversion oniguruma)
        echo "Found oniguruma version: $onig_version"
    fi
    
    # Check cmake version (minimum 3.5 for modern builds)
    if command -v cmake >/dev/null 2>&1; then
        cmake_version=$(cmake --version | head -1 | grep -o '[0-9]\+\.[0-9]\+')
        echo "Found cmake version: $cmake_version"
        if ! echo "$cmake_version" | grep -Eq '^([3-9]\.[5-9]|[4-9]\.|[1-9][0-9]\.)'; then
            echo "Warning: cmake version $cmake_version may be too old (requires 3.5+)"
        fi
    else
        echo "cmake not found - will be installed"
    fi
    
    # Check OpenSSL version (minimum 1.1.1 for PHP 8.4)
    if command -v openssl >/dev/null 2>&1; then
        ...
        if ! echo "$openssl_version" | grep -Eq '^(1\.1\.[1-9]|1\.[2-9]\.|[2-9]\.)'; then
            echo "Warning: OpenSSL version $openssl_version may be too old for PHP 8.4 (requires 1.1.1+)"
        fi
    fi
}
